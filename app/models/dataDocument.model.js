...
module.exports = class DataDocument {
  ...
  async submit(connection) {
    debug(`Document submit. Tries so far: ${this._submitTries}`);
    ...
    try {
      ...
      debug(
        `Unable to submit document after ${this._submitTries} attempts`,
      );
  ...
  static async find(connection, locator, query) {
    debug(
      `find documents with locator ${locator} matching query ${JSON.stringify(
        query,
      )}.`,
    );
  ...
  static async waitFor(
    ...
    try {
      ...
      if (timeErr < timeEnd) {
        ...
        debug(
          `Attempt to retsrt waitFor for remaining ${timeRemaining}ms`,
        );
        await this.waitFor(
          connection,
          locator,
